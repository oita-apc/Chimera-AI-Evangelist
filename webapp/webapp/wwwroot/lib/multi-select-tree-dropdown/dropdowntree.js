        ...
        for(var i = 0 ; i< data.length ; i++){
            ...
            if(typeof data[i].dataAttrs != "undefined" && data[i].dataAttrs != null){
                for(var d = 0 ; d<data[i].dataAttrs.length ; d++){
                    ...
                    if (data[i].dataAttrs[d].title === "checked" && data[i].dataAttrs[d].data === true) {
                        checked = true;                        
                    }
                }
            }

            var square_o = "fa-square-o";
            if (checked) {
                square_o = "fa-check-square-o";
            }

            if(!element.is("li")){
                element.append('<li id="TreeElement'+globalTreeIdCounter+'"'+dataAttrs+'>'+(options.multiSelect?'<i class="fa ' + square_o + ' select-box" aria-hidden="true"></i>':'')+'<a>'+data[i].title+'</a></li>');
                ...
                element.find("ul").append('<li id="TreeElement'+globalTreeIdCounter+'"'+dataAttrs+'>'+(options.multiSelect?'<i class="fa ' + square_o + ' select-box" aria-hidden="true"></i>':'')+'<a>'+data[i].title+'</a></li>');
                if(data[i].data != null && typeof data[i].data !="undefined"){
                    $("#TreeElement"+globalTreeIdCounter).append("<ul style='display:none'></ul>");
                    $("#TreeElement"+globalTreeIdCounter).find("a").first().prepend('<span class="arrow">'+options.closedArrow+'</span>');
                    RenderData(data[i].data, $("#TreeElement"+globalTreeIdCounter).find("ul").first());
